...
def clear_group(switch_id, group_id):
	if isinstance(group_id, int): # if input is single flow id
		group_id = [group_id]
	
	for group in meter_id:
		url = "http://localhost:8181/restconf/config/opendaylight-inventory:nodes/node/openflow:"+str(switch_id)+"/group/"+str(group)
		payload = ""
		response = requests.request("DELETE", url, data=payload, headers=headers)
		handle_response(response)
