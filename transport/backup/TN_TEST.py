...
PRIORITY = 1+1  # global variable

#the ruckus switch openflow id #'s' # XXX find the correct switch id XXX
SWITCH_ID = 106225813884292 

PORT_LISTS = [12, 1] # 12 ports in switch, do not broadcast to all ports, that reduces performance

TABLE_ID = 0  # default table id
GROUP_ID = 1  # default broadcast group id for arp flows

_, ORDERED_USERS, _, _ = parse_database(DATABASE) # get default users and bandwidth
...
def implement_configuration(switch_id, static_users, regular_users, bandwidth, table_id=0):
    ...
    for i in range(len(static_users)):
        ...
        create_network(switch_id, static_users[i], static_flow[idx], -1, 12, 1, PRIORITY, table_id)  # for both bs1 and bs2
    ...
    for i in range(len(regular_users)):
        ...
        flow_matrix[idx] = create_network(switch_id, regular_users[i], flow_matrix[idx], meter_matrix[idx], 12, 1, PRIORITY, table_id) # XXX port hard code
        # TODO make sure to connect ethenet cable between port 5 and 7
        # flow_matrix[idx] = create_network(switch_id, regular_users[i], flow_matrix[idx], meter_matrix[idx], 7, 12, PRIORITY, table_id) # XXX port hard code
...
# add_arp_broadcast(SWITCH_ID, 1) # TODO fixme 1
for i in range(len(PORT_LISTS)):
    add_flow_arp(SWITCH_ID, PORT_LISTS[i], i+1, TABLE_ID, PORT_LISTS[1-i]) # XXX TODO

# create constant path for base station 1 and 2 toward core network # TODO fixme 2, 4
for i in range(len(const_hosts)):
    create_network(SWITCH_ID, const_hosts[i], [2*(i+1)], -1, PORT_LISTS[0], PORT_LISTS[1], 1)  
